...
CI (`.github/workflows/ci.yml`) runs lint, typecheck, `test:coverage`, and build as separate steps on Node 22 and 24, and uploads coverage and production-bundle artifacts (`npm run check` is the equivalent local one-shot). The Security workflow gates on `npm audit --omit=dev --audit-level=high` (shipped dependencies only; a full audit runs report-only). CodeQL and dependency review are opt-in via the `ENABLE_GHAS_CHECKS` repository variable — they need GitHub Advanced Security while the repo is private — and CodeQL is advisory unless branch protection requires its check. The live WalletConnect relay test is intentionally manual because it requires the `VITE_WC_PROJECT_ID` repository secret and external relay availability.
